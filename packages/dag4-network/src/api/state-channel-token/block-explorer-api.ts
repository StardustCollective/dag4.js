...
/**
 * @todo
 * Add support to the state channel block explorer API
 */
class StateChannelTokenBlockExplorerApi {
  private service = new RestApi("");

  constructor(host?: string) {
    throw new Error(
      "BlockExplorerApi for state channel tokens is not yet supported :["
    );
    if (host) {
  ...
  async getTransactionsBySnapshot(id: HashOrOrdinal) {
    return this.service.$get<TransactionV2[]>(
      `/global-snapshots/${id}/transactions`
    );
  }

  async getRewardsBySnapshot(id: HashOrOrdinal) {
    return this.service.$get<RewardTransaction>(
      `/global-snapshots/${id}/rewards`
    );
  }
...
export { StateChannelTokenBlockExplorerApi };
